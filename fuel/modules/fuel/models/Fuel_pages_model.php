...
class Fuel_pages_model extends Base_module_model {
	...
	function list_items($limit = NULL, $offset = NULL, $col = 'last_modified', $order = 'desc', $just_count = FALSE)
	{
		$CI =& get_instance();

		if (!$CI->fuel->auth->is_super_admin())
		{
			$this->db->where(array('assign_role' => $CI->fuel->auth->get_role_id()));
		}
		$this->db->join($this->_tables['fuel_user_role'], $this->_tables['fuel_user_role'].'.id = '.$this->_tables['fuel_pages'].'.assign_role', 'left');

		$this->db->select('fuel_pages.id, location, layout, user_role.name AS assign_role, last_modified, published');
	...
	public function form_fields($values = array(), $related = array())
	{
		...
		$fields['assign_role'] = array(
				'label' => 'Assign Department',
				'type' => 'inline_edit', 
				'module' => 'user_role',
				'options' => $CI->fuel_user_roles_model->options_list(),
				'comment' => 'The grouping of Department you want to associate this page to',
				'order' => 3
		);

		if (!$CI->fuel->auth->is_super_admin())
		{
			unset($fields['assign_role']);
			$fields['location']['comment'] = "Only Administror role can edit this location";
		}

		$fields['layout']['type'] = 'select';
		$fields['layout']['options'] = $CI->fuel->layouts->options_list();
		$fields['layout']['order'] = 4;
		...
		{
			$fields['navigation_label'] = array(
				'comment' => lang('navigation_quick_add'),
				'order' => 2
			);
		}
	...
	public function on_before_validate($values)
	{
		if (!empty($values['id']))
		{
			...
			if (!$CI->fuel->auth->is_super_admin())
			{
				$this->add_validation('id', array(&$this, 'is_assigned_page'), 'You don\'t have permission to edit this page.');
	...
	/**
	 * Determines whether a user has edit permission/assigned page
	 *
	 * @access	public
	 * @param	string The page id
	 * @return	boolean 
	 */	
	public function is_assigned_page($id)
	{
		$CI =& get_instance();
		return $this->record_exists(array('id' => $id, 'assign_role' => $CI->fuel->auth->get_role_id()));
