...
class Important_updates_model extends Base_module_model
{

	public $required = array('title');
	
	public $record_class = 'Important_updates_model_item';
	

	function __construct()
	{
		parent::__construct('important_updates'); // table name
		//echo 'notification'; die;
	}

	function list_items($limit = NULL, $offset = NULL, $col = 'publish_till', $order = 'desc', $just_count = false)
	{
		$this->db->select('id,title, message, pdf, publish_till, published');
	...
	public function on_before_validate($values)
	{
		...
		if (!empty ($values['title'])) {
			$title = $values['title'];
			if (!preg_match('/^[A-Za-z0-9\x{0900}-\x{097F}\-\/, ]+$/u', $title)) {
				$this->add_error('Only alphabetical and hindi characters allowed in title', 'title');
				return;
			}
		}
...
class Important_updates_model_item extends Base_module_record
{
