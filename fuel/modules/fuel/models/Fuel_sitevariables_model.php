...
class Fuel_sitevariables_model extends Base_module_model {
	...
	public function form_fields($values = array(), $related = array()){
		...
		$fields['value']['type'] = 'varchar';
		unset($fields['value']['editor']);
		return $fields;
	}
	public function on_before_validate($values)
	{
		...
		if (!empty ($values['value'])) {
			...
			if (!ctype_alnum($value)) {
				$this->add_error('Only alphanumeric values allowed in value.', 'value');
				return;
			}
		}
		if (!empty ($values['scope'])) {
			$scope = $values['scope'];
			if (!preg_match('/^[a-zA-Z-]*$/', $scope)) {
				$this->add_error('Only alphabetical characters allowed in scope', 'scope');
				return;
			}
		}
		$active = $values['active'];
		if (!in_array($active, ['yes', 'no'])) {
			$this->add_error('Invalid active.', 'active');
			return;
		}
		return $values;
	}
