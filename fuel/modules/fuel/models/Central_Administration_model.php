...
class Central_Administration_model extends Base_module_model
{

	public $required = array('name','image');

	public $record_class = 'Central_Administration_model_item';
	//public $filters = array('published'=>'yes'); // the default list view group value for filtering

	function __construct()
	{
		parent::__construct('central_administration'); // table name
		// echo 'bharat'; die;
	}

	function list_items($limit = NULL, $offset = NULL, $col = 'seqno', $order = 'asc', $just_count = false)
    {
    $this->db->select('id, email, name, designation, qualification, subject, experience, image, seqno, published');
    $data = parent::list_items($limit, $offset, $col, $order);

    foreach ($data as $key => $value) {
        $data[$key]['image'] = '<img src="' . img_path('central_administration/' . $value['image']) . '" alt="" style="width: 100px; height: 100px;" />';
    }
	...
	function form_fields($values = array(), $related = array())
	{
		...
		$fields['image'] = array(
			...
			'type' => 'asset',
			'multiple' => FALSE,
			'folder' => 'images/central_administration',
			'hide_options' => TRUE,
...
class Central_Administration_model_item extends Base_module_record
{

	public function get_image_path()
	{
		return img_path('central_administration/' . $this->image);
	}
}
?>
