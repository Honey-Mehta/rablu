...
class Fuel_users_model extends Base_module_model {
	...
	public function valid_user($user, $pwd)
	{
		...
		//overwritten : mpo166
		$session_salt = !empty($_SESSION[$this->fuel->auth->get_session_namespace()]['ci_csrf_token_CMS']) ? $_SESSION[$this->fuel->auth->get_session_namespace()]['ci_csrf_token_CMS'] : NULL;
		
		if ($pwd == hash('sha256', $user['password'] . $session_salt))
		{
			return $user;
		} else {
			return FALSE;
	...
	public function list_items($limit = NULL, $offset = NULL, $col = 'email', $order = 'desc', $just_count = FALSE)
	{
		...
		$this->db->select($this->_tables['fuel_users'].'.id, email, user_name, first_name, last_name, super_admin, '.$this->_tables['fuel_users'].'.active,'. $this->_tables["fuel_user_role"].'.name AS department');
		$this->db->join($this->_tables['fuel_user_role'], $this->_tables['fuel_user_role'].'.id = '.$this->_tables['fuel_users'].'.role_id', 'left');
	...
	public function salted_password_hash($password, $salt)
	{
		...
		//overwritten: return without server side salt : mpo166
		return $password;
	}
	...
	public function form_fields($values = array(), $related = array())
	{
			...
			)
		{
			...
			$role_options = $CI->fuel_user_roles_model->options_list();
			...
			$fields['role_id'] = array(
				...
				'comment' => 'The grouping of Department you want to associate this user to',
				'order' => 2
			);
	...
	public function on_before_validate($values)
	{
		...
		if (!empty ($values['first_name'])) {
			...
			if (!preg_match('/^[a-zA-Z]*$/', $first_name)) {
				$this->add_error('Only alphabetical characters allowed in first name', 'first_name');
				return;
			}
		}
		if (!empty ($values['last_name'])) {
			...
			if (!ctype_alnum($last_name)) {
				$this->add_error('Only alphanumeric characters allowed in last name', 'last_name');
				return;
			}
		}
