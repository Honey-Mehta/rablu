...
class Placements_model extends Base_module_model {

	public $required = array('name','post','mobile','passing_year','firm_name');
	public $filters = array('group_id'); // the default list view group value for filtering
	public $record_class = 'placement_item';
	//public $parsed_fields = array('content');
	
	function __construct()
	{
		parent::__construct('tbl_placements'); // table name
	}
	...
	function list_items($limit = NULL, $offset = NULL, $col = 'created', $order = 'desc', $just_count = false)
	{
		...
		$this->db->select('user_role.name AS Department,tbl_placements.name,post,mobile,email,passing_year,firm_name,published,'.$this->_tables['tbl_placements'].'.id');
		$this->db->join($this->_tables['fuel_user_role'], $this->_tables['fuel_user_role'].'.id = '.$this->_tables['tbl_placements'].'.group_id', 'left');
	...
	{	
		...
		if (!$CI->fuel->auth->is_super_admin())
		{
			...
			$fields['group_id'] = array(
				...
				'comment' => 'The grouping of Department you want to associate this student to',
				'required'=>TRUE,
				'order' => 3
			);
		}
	...
	public function on_before_validate($values)
	{
		...
		if (!empty($values['id']))
		{
			...
			if (!$CI->fuel->auth->is_super_admin())
			{
				$this->add_validation('id', array(&$this, 'is_assigned_alumni'), 'You don\'t have permission to edit this Student Information.');
...
class Placement_item_model extends Base_module_record {
