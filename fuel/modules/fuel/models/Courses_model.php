...
class Courses_model extends Base_module_model {
	...
	public $required = array('course_name','eligibility','available_seat');
	public $filters = array('group_id'); // the default list view group value for filtering
	public $record_class = 'course_item';
	//public $parsed_fields = array('content');
	
	function __construct()
	{
		parent::__construct('tbl_course'); // table name
	}
	...
	function list_items($limit = NULL, $offset = NULL, $col = 'created', $order = 'desc', $just_count = false)
	{
		...
		$this->db->select('user_role.name AS Department,course_name,mode,course_type,duration,available_seat,eligibility,published,'.$this->_tables['tbl_course'].'.id');
		$this->db->join($this->_tables['fuel_user_role'], $this->_tables['fuel_user_role'].'.id = '.$this->_tables['tbl_course'].'.group_id', 'left');
	...
	{	
		...
		$fields['mode']['options'] = array('Regular Course' => 'नियमित पाठ्यक्रम', 'Self Supporting Course' => 'स्व सहायक कोर्स', 'Correspondence Course' => 'पत्राचार पाठ्यक्रम');
	 ...
	 * @return	boolean 
	 */	
	public function is_assigned_course($id)
	{
		$CI =& get_instance();
	...
	public function on_before_validate($values)
	{
		...
		if (!empty($values['id']))
		{
			...
			if (!$CI->fuel->auth->is_super_admin())
			{
				$this->add_validation('id', array(&$this, 'is_assigned_course'), 'You don\'t have permission to edit this course.');
	...
	function _common_query($display_unpublished_if_logged_in = NULL)
	{
		...
		$this->db->select('user_role.name AS Department,'.$this->_tables['tbl_course'].'.*');
		$this->db->join($this->_tables['fuel_user_role'], $this->_tables['fuel_user_role'].'.id = '.$this->_tables['tbl_course'].'.group_id', 'left');
		$this->db->order_by('user_role.id asc');
	}
}

class Course_item_model extends Base_module_record {
