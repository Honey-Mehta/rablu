...
class Photo_model extends Base_module_model
{

	public $required = array('image');

	public $record_class = 'Photo_model_item';
	//public $filters = array('published'=>'yes'); // the default list view group value for filtering

	function __construct()
	{
		parent::__construct('photo'); // table name
		// echo 'bharat'; die;
	}
    ...
    foreach ($data as $key => $value) {
        $data[$key]['image'] = '<img src="' . img_path('gallery/' . $value['image']) . '" alt="" style="width: 100px; height: 100px;" />';
    }
...
class Photo_model_item extends Base_module_record
{

	public function get_image_path()
	{
		return img_path('gallery/' . $this->image);
	}
}
?>
