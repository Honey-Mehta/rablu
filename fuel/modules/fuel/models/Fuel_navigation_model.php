...
class Fuel_navigation_model extends Base_module_model {
	...
	//	$this->add_validation('id', array(&$this, 'no_id_and_parent_match'), lang('error_location_parents_match'), $values['parent_id']);
		...
		// 	$label = $values['label'];
		// 	if (!preg_match('/^[A-Za-z0-9\x{0900}-\x{097F}\-\/, ]+$/u', $label)) {
		// 		$this->add_error('Only alphanumeric and hindi characters allowed in label', 'label');
		// 		return;
		// 	}
		// }
		if (!empty ($values['precedence'])) {
			$precedence = $values['precedence'];
			if (!ctype_digit($precedence)) {
				$this->add_error('Only Numeric value allowed in precedence', 'precedence');
				return;
			}
		}
		return $values;
