<style type="text/css">div#dashboard_fuel {float: none;}</style>
<?php if ($this->fuel->auth->has_permission('logs')) : ?>
<?php if (!empty($latest_activity)) : ?>
<div class="dashboard_pod" style="width: 95%;">
...
<div class="dashboard_pod" style="width: 45%;">
	<h3><?=lang('dashboard_hdr_modified')?></h3>
		<ul class="nobullets">
			<?php foreach($recently_modified_pages as $val) : ?>
			<li><a href="<?=base_url($val['location'])?>" target="_blank" ><?=$val['location']?></a></li>
			<?php endforeach; ?>
		</ul>
...
<div class="dashboard_pod" style="display:none; width: 230px;">
