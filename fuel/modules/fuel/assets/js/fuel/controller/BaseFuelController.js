	...
	add_edit : function(initSpecFields){
		...
		$(document).on('click', '.save, #form input[type="submit"]', function(e){
            ...
            if (newPass != '' && newPass == oldPass) {
                _this.setNotification('Old and New password must not be same.', 'warning', 'rearrange');
                return false;
            }

            if (newPass != '') {
                if (!_this.checkPassword(newPass)) {
                    ...
                    _this.setNotification(msg, 'warning', 'rearrange');
                    return false;
                }
                // $('#new_password').val(_this.preparePassword(sha256(newPass)));
                $('#new_password').val((sha256(newPass)));
            }
            if (confirmPass != '') {
                // $('#confirm_password').val(_this.preparePassword(sha256(confirmPass)));
                $('#confirm_password').val((sha256(confirmPass)));
            }
            if (oldPass != '') {
                var token = $("#ci_csrf_token_CMS").val();
                // $('#old_password').val(token + _this.preparePassword(sha256(oldPass)));
                $('#old_password').val((sha256(sha256(oldPass) + token)));
            }
